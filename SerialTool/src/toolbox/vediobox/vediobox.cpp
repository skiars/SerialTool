...
void VedioBox::retranslate()
{
    ui->retranslateUi(this);
}

void VedioBox::receiveData(const QByteArray &array)
{
    m_array.append(array);
    qint32 i, len = m_array.size() - 1;
    const char *data = m_array.data();
    ...
    if (i != 0 && i < len) { // 匹配到内容
        m_array = m_array.mid(i); // 保留余下内容
    }
    // 检查是否接收到一幅图像
    if ((quint8)m_array.data()[0] == 0x0B
        && (quint8)m_array.data()[1] == 0xBB
        && m_array.size() >= 602) {
        ...
        image = pixmap; // 保存图像
        memcpy(imageData, m_array.data() + 2, 600); // 复制到缓冲区
        m_array = m_array.mid(602); // 保留余下内容
    }
}
